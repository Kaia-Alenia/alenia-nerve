...
### Fixed
- LAN discovery no longer depends on DNS or an Internet-backed default route when identifying local IPv4 interfaces.
- Discovery responses now include the host's stable peer identity.

### Changed
- Synchronized the Python, JavaScript, and Rust package versions at `7.0.0`.
...
### Changed
- Synchronized version to `1.5.6` across all client packages (Python, JavaScript/npm, Rust/crates.io).
...
- Added `nerve genpass` command and interactive password generation during `.nrv` packing, featuring an EFF diceware passphrase generator.

## [1.5.1] — 2026-07-20

### Fixed
- Python: `nerve dashboard` returned 404 because `dashboard/index.html` was not
  included in the published wheel. Fixed by declaring the file in
