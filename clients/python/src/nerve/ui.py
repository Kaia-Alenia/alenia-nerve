...
def show_error(message: str) -> None:
    ...
    if system == "Darwin":
        try:
            ...
            subprocess.run(["osascript", "-e", f'display alert "{escaped_msg}" as critical'], check=False)
        except Exception:
            pass
    elif system == "Windows":
        try:
        ...
        try:
            result = subprocess.run(["zenity", "--error", "--text", message, "--title", "Error"])
...
def prompt_password(prompt: str, error: str | None = None) -> str:
    ...
    if system == "Darwin":
        ...
        try:
            result = subprocess.run(["zenity", "--password", "--title", "Nerve"], capture_output=True, text=True, check=True)
            return result.stdout.strip("\n\r")
