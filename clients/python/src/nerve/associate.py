...
def _associate_windows() -> bool:
    try:
        ...
        # Paths
        ico_path = str(importlib.resources.files('nerve.icons').joinpath('nrv-icon.ico'))
        python_exe = get_python_exe()
        command = f'"{python_exe}" -m nerve.cli open "%1"'
        
        # Register .nrv
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, r"Software\Classes\.nrv") as key:
            winreg.SetValue(key, "", winreg.REG_SZ, "NerveFile")
            
        # Register NerveFile
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, r"Software\Classes\NerveFile") as key:
            winreg.SetValue(key, "", winreg.REG_SZ, "Nerve Secure Container")
            
        # DefaultIcon
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, r"Software\Classes\NerveFile\DefaultIcon") as key:
            winreg.SetValue(key, "", winreg.REG_SZ, f'"{ico_path}"')
            
        # Command
        with winreg.CreateKey(winreg.HKEY_CURRENT_USER, r"Software\Classes\NerveFile\shell\open\command") as key:
...
"""
        ...
        try:
            subprocess.run(["xattr", "-d", "com.apple.quarantine", app_dir], stderr=subprocess.DEVNULL, check=False)
        except Exception:
            pass
...
"""
        ...
        for size in sizes:
            icon_dir = os.path.join(home, ".local", "share", "icons", "hicolor", f"{size}x{size}", "mimetypes")
...
"""
        ...
        # Update caches
        try:
            subprocess.run(["update-mime-database", os.path.join(home, ".local", "share", "mime")], check=False)
        except Exception:
            pass
            
        try:
            subprocess.run(["gtk-update-icon-cache", "-f", "-t", os.path.join(home, ".local", "share", "icons", "hicolor")], check=False)
        except Exception:
            pass
            
        try:
            subprocess.run(["xdg-mime", "default", "nerve.desktop", "application/x-nerve"], check=False)
        except Exception:
            pass
...
def _unassociate_linux() -> bool:
    try:
        ...
        # Remove MIME type
        mime_file = os.path.join(home, ".local", "share", "mime", "packages", "nerve.xml")
        ...
        sizes = [16, 32, 64, 128, 256]
        for size in sizes:
            icon_file = os.path.join(home, ".local", "share", "icons", "hicolor", f"{size}x{size}", "mimetypes", "application-x-nerve.png")
            if os.path.exists(icon_file):
                os.remove(icon_file)
                
        # Remove .desktop file
        desktop_file = os.path.join(home, ".local", "share", "applications", "nerve.desktop")
