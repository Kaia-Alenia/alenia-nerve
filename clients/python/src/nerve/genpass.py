...
def generate_passphrase(words: int = 5, separator: str = "-", append_digits: int = 0) -> tuple[str, float]:
    """
    ...
    text_content = importlib.resources.files('nerve').joinpath('eff_large_wordlist.txt').read_text(encoding='utf-8')
    wordlist = [line.strip() for line in text_content.splitlines() if line.strip() and not line.startswith('#')]
