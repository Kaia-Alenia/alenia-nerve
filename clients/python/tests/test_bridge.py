...
def test_bridge_initialization(mock_nexus_client):
    bridge = NerveBridge(host="10.0.0.1", port=1234, allowed_origins=["http://localhost"])
...
async def test_ws_handler_and_hub_message(mock_nexus_client):
    ...
    class MockWebsocket:
        ...
        async def __aiter__(self):
            ...
            yield 'invalid json'
            raise websockets.exceptions.ConnectionClosedOK(websockets.frames.Close(1000, ""), websockets.frames.Close(1000, ""), False)
    ...
    assert mock_websocket.sent_messages == ['{"bridge_client_id": "' + ws_id + '", "data": "back"}']
