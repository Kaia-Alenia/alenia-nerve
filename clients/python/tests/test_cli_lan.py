...
def test_cli_scan() -> None:
    with patch("sys.argv", ["nerve", "scan"]), \
...
def test_cli_diagnose() -> None:
    with patch("sys.argv", ["nerve", "diagnose", "127.0.0.1"]), \
...
def test_cli_send() -> None:
    with patch("sys.argv", ["nerve", "send", "some/path", "--to", "10.0.0.1"]), \
...
def test_cli_receive() -> None:
    with patch("sys.argv", ["nerve", "receive", "--dir", "/tmp"]), \
