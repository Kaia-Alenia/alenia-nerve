# Registro de Cambios — alenia-nerve (Python)
...
## [7.0.0] — 2026-09-09
### Cambiado
- Sincronizada la versión del cliente Python con la release principal de Nerve.
- Mejorado el descubrimiento LAN para funcionar sin DNS ni Internet.
...
## [1.5.6] — 2026-07-31
### Corregido
- **Core** (`core.py`): Corregido bug crítico en `NexusHub.start()` donde `raise OSError` por "dirección ya en uso" era capturado por su propio bloque `except`, causando que siempre se intentara eliminar el socket aunque hubiera un hub activo.
- **Bridge** (`bridge.py`): Corregido bloqueo infinito en `NerveBridge.start()` cuando el hub no estaba corriendo. Implementado routing real en `_handle_hub_message` con `asyncio.run_coroutine_threadsafe`. Corregido `KeyError` en `_ws_handler` cleanup usando `discard()` y `.pop()` seguros.
- **CLI Monitor** (`cli_monitor.py`): `nerve dashboard` y `nerve monitor` ya no se cuelgan indefinidamente cuando el hub no está activo. Ahora muestran un error claro y salen inmediatamente.
### Cambiado
...
### Cambiado
- Incrementada la versión del paquete para asegurar una publicación limpia en PyPI y sincronización de descripción.
...
- `NexusClient`: Se evitaron bucles de reconexión infinitos al desconectarse explícitamente.
- `NexusHub`: Se evitaron fugas de hilos y sockets al detener cerrando todos los sockets de clientes activos.
...
- `CONTRIBUTORS.md` — archivo de reconocimiento de contribuidores.
...
### Añadido
- Comando CLI `nerve start` inicial.
...
- `NexusHub` — hub de enrutamiento central a través de Unix Domain Socket o TCP.
